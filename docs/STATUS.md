...
| MikroTik provisioning | Hashed `prv_` bootstrap tokens with TTL and revoke. Short paste script checks internet and fetches `.rsc` over HTTPS with `check-certificate=yes`. Public fetch URL comes from the domain resolver (verified custom → verified subdomain → central `app.public_url`). Placeholder hosts such as `YOUR-PUBLIC-URL` are never generated. Config versions and events stored. Unique overlay IPs. Online is not claimed without heartbeat/pull. The Routers desk is a compact summary list; details, IP pools, and monitoring load only after a router is opened (`router-desk.test.ts`). Proof: `router-provisioning.test.ts`, `domain-resolve.test.ts`, `access.test.ts`. Live kernel handshake is **not** run in CI. |
