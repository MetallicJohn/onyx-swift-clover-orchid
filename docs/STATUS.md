...
| MikroTik agent | Enroll token + generated WG keys. Peer endpoint is `wg.ispsolutions.co.ke:51820` with `allowed-address=10.200.0.1/32`. Dedicated `ispsolutions-agent` API user on TCP 8728 (no API-SSL, no WAN). HTTPS fetch uses `check-certificate=no`. Enrollment states are explicit; ENROLLED requires handshake + API + agent. Proof: `router-enroll-state.test.ts`, `mikrotik.test.ts`. |
| MikroTik provisioning | Hashed `prv_` bootstrap tokens with TTL and revoke. Short paste script checks internet and fetches `.rsc` over HTTPS with `check-certificate=no` (MikroTik often has no CA store). Public fetch URL comes from the domain resolver (verified custom → verified subdomain → central `app.public_url`). Placeholder hosts such as `YOUR-PUBLIC-URL` are never generated. Config versions and events stored. Unique overlay IPs. Online is not claimed without heartbeat/pull. The Routers desk is a compact summary list; details, IP pools, and monitoring load only after a router is opened (`router-desk.test.ts`). Proof: `router-provisioning.test.ts`, `domain-resolve.test.ts`, `access.test.ts`. Live kernel handshake is **not** run in CI. |
| Billing lifecycle | Overdue past `grace_days` suspends. Inside grace → `grace`. Expired `period_end` suspends (`time`) and sends Service Expired. Data cap suspends (`bundle`). Full payment extends the period, resets usage, restores unless another invoice is still overdue. Optional partial payment (off by default) grants pro-rata validity when the configured minimum % is met, for that service only. Business/enterprise packages with a configured maximum credit stay Active after expiry while that **service's** outstanding is below the limit (`business_credit`); they suspend at the limit (`credit_limit`) and invoices stack while below it. New services default expiry to today when awaiting payment, or today + package validity when started as Active. Continuing / migrated services keep the selected existing expiry as the first renewal (`billing_anchor_date`); import does not invoice or send onboarding SMS. Date fields (subscription start, expiry, due dates) enter **dd/mm/yy** by default; storage is YYYY-MM-DD (`display.test.ts`). Proof: `onboard.test.ts`, `onboard-notify.test.ts`, `onboard-import.test.ts`, `partial-payment.test.ts`, `business-credit.test.ts`. Optional exclusive VAT. Platform plans activate after M-Pesa, not on click. |
...
| Password reset | Operator/superadmin email reset (hashed token, 30 min), including Google/X-only accounts that then get an email password. Portal password + SMS reset. Admin/staff can set a password. Email sends only with tenant SMTP/Resend; otherwise the link is shown. |
...
| VPS publish | Docker Compose + Caddy + Postgres. Deploy only after green CI on `main`: verified custom `pg_dump`, destructive-SQL scan, additive migrate under advisory lock, health, row-count compare, **code** rollback (DB not auto-restored). Timer fetches GitHub but does not rebuild unless `ISPSOLUTIONS_AUTO_DEPLOY=1`. Never `compose down -v`. Proof: `migration-safety.test.mjs`, `data-integrity.test.mjs`, `deploy-protect.test.mjs`. Platform deploy remains Vercel. |
