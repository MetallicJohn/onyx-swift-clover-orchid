...
| MikroTik provisioning | Hashed `prv_` bootstrap tokens with TTL and revoke. Short paste script checks internet and fetches `.rsc` over HTTPS with `check-certificate=yes`. Config versions and events stored. Unique overlay IPs. Online is not claimed without heartbeat/pull. Proof: `router-provisioning.test.ts`. Live kernel handshake is **not** run in CI. |
