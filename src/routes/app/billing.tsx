...
function BillingPage() {
  ...
  async function load(selectId?: string) {
    ...
    const fmt = res.workspace.dateFormat || "dd/mm/yy";
    setDateFormat(fmt);
  ...
  useEffect(() => {
    ...
    load().catch((ex) => setLoadError(ex instanceof Error ? ex.message : "Could not load billing"));
  ...
  return (
    <div className="space-y-6">
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setCycleNote(`Billing cycle issued ${r.issued} invoice${r.issued === 1 ? "" : "s"}.`);
                  await load();
                } catch (ex) {
                  setErr(ex instanceof Error ? ex.message : "Billing cycle failed");
                } finally {
                  setBusy(false);
                }
              }}
      ...
      {loadError ? <p className="text-sm text-danger print:hidden">{loadError}</p> : null}
      {cycleNote ? <p className="text-sm text-accent print:hidden">{cycleNote}</p> : null}
...
function quotesForCustomer(quotes: Quote[], customerId: string): Line[] {
  ...
  return q.map((x) => ({
    description: `${x.package_name} (${x.billing_interval})${x.account_number ? ` · ${x.account_number}` : ""}`,
    quantity: 1,
