...
export async function syncRadiusAccount(
  ...
  if (existing[0]) {
    await sql`update radius_accounts
      set username = ${username}, framed_ip = ${service.static_ip ?? ""}, group_name = ${group}, enabled = ${enabled}
      where id = ${existing[0].id}`;
    return { username, password: existing[0].password, enabled };
  }
  const password = secret();
  await sql`insert into radius_accounts (id, tenant_id, service_id, username, password, framed_ip, group_name, enabled)
    values (${nid("rad")}, ${tenantId}, ${service.id}, ${username}, ${password}, ${service.static_ip ?? ""}, ${group}, ${enabled})`;
