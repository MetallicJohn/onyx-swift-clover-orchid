...
test("bootstrap paste checks internet, fetches HTTPS, and skips MikroTik CA verify", () => {
  ...
  assert.match(script, /dst-path="ispsolutions-bootstrap.rsc"/);
  assert.match(script, /mode=https check-certificate=no/);
  assert.match(script, /output=file dst-path="ispsolutions-bootstrap.rsc" as-value/);
  assert.match(script, /https:\/\/ops\.imani\.ke\/api\/vpn\/routers\//);
  assert.match(script, /\/import file-name=\$ispSolFile/);
  assert.match(script, /ispSolHaveFile/);
  assert.doesNotMatch(script, /HTTPS fetch failed — check URL/);
  assert.doesNotMatch(script, /:foreach i in=\[\/file find\]/);
  assert.doesNotMatch(script, /:find \$n /);
