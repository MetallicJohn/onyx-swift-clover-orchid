...
echo "[ispsolutions-radius] waiting for ISP Solutions at $BASE"
i=0
while [ "$i" -lt 40 ]; do
...
echo "[ispsolutions-radius] pulling bootstrap for $SLUG"
BOOT="$(curl -fsS --max-time 15 -u "ispsolutions:${KEY}" "${BASE}/api/v1/radius/bootstrap/${SLUG}")"
printf '%s' "$BOOT" | jq -r .rest >"$RADDB/mods-available/rest"
printf '%s' "$BOOT" | jq -r .site >"$RADDB/sites-available/ispsolutions"
