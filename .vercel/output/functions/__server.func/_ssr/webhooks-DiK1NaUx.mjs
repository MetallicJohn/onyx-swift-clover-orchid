import { b as nid, r as applyRls } from "./rls-stkZtAMF.mjs";
import { t as applyConfirmedPayment } from "./payments-D3oS_eip.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/webhooks-DiK1NaUx.js
...
async function handleMpesaCallback(slug, body) {
	const { getSql } = await import("./db-BsfBburB.mjs").then((n) => n.t).then((n) => n.t);
...
async function handleKopokopoCallback(slug, body) {
	...
	await settleFromDecision(sql, tenant, "kopokopo", intent, {
		...parsed,
		checkout: intent.checkout_id
	});
	return { ok: true };
}
