...
import { A as slugify, C as provisionServiceAccess, P as wgAddressForIndex, b as nid, d as emit, f as enqueueAgentCommand, j as syncRadiusAccount, k as seedOpsForTenant, m as enrollFields, n as agentScript, r as applyRls, t as agentPullUrl } from "./rls-stkZtAMF.mjs";
import { t as authMiddleware } from "./middleware-BuXiR3_1.mjs";
import { i as getSql } from "./db-BsfBburB.mjs";
import { t as applyConfirmedPayment } from "./payments-D3oS_eip.mjs";
import { n as listInbox } from "./inbox-C11LOMfj.mjs";
import { i as runBillingCycle, n as issueInvoice, r as notifyCustomerEvent, t as ensureDefaultTemplates } from "./notifications-NHmcUGyA.mjs";
import { n as resolveActiveTenant, r as setActiveTenant, t as assertPermission } from "./tenant-context-BsYaB9rN.mjs";
import { t as assertCustomerQuota } from "./saas-CEVRi20s.mjs";
import { i as openTicket } from "./tickets-DgGBky5M.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/server-CSL7e86Y.js
...
async function ensureWorkspace(sql, userId, displayName, email) {
	...
	if (active) {
		...
		const [row] = await sql`select demo_seeded from tenants where id = ${active.tenantId}`;
		if (!row?.demo_seeded) {
			await applyRls(sql, { bypass: true });
...
var listMyTenants = createServerFn({ method: "GET" }).middleware([authMiddleware]).handler(listMyTenants_createServerFn_handler, async ({ context }) => {
	const sql = await getSql();
	const { listMemberships } = await import("./tenant-context-BsYaB9rN.mjs").then((n) => n.i).then((n) => n.r);
...
var createService = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((d) => d).handler(createService_createServerFn_handler, async ({ context, data }) => {
	...
	if (!(await sql`
      ...
      and status in ('issued','due','overdue') limit 1`)[0] && pkg.price_kes > 0) {
